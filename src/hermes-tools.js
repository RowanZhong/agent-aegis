...
function translateHermesTool(toolName, args, ctx) {
    const params = { ...args };
    switch (toolName) {
        case "terminal":
        ...
        case "skill_manage": {
            const skillDir = ctx.skillDir ?? path.join(ctx.home, "skills", typeof params.category === "string" ? params.category : "", typeof params.name === "string" ? params.name : "");
            const target = params.action === "delete" ? skillDir : path.resolve(skillDir, typeof params.file_path === "string" && params.file_path ? params.file_path : "SKILL.md");
...
export function normalizeHermesTool(toolName, args, ctx) {
