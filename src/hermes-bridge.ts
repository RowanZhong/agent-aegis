...
import { TOOL_CALL_DEFENSE_STRATEGIES, type ToolCallDefenseStrategy } from "./security-strategies.js";
...
function hostPolicyText(text: string): string {
  return text.replace(/\bhermes\b/gi, "openclaw");
}

const hermesCommandGuard: ToolCallDefenseStrategy = {
...
export function createHermesBridge() {
  let runtime: ReturnType<typeof createClawAegisRuntime> | undefined;
  let home = "";
  let promptGuardEnabled = false;
  const deniedSessions = new Map<string, string>();
  const log = (message: string) => process.stderr.write(`${message}\n`);
  return {
    async dispatch(method: string, payload: Payload): Promise<unknown> {
      if (method === "initialize") {
        if (runtime) throw new Error("AgentAegis is already initialized");
      ...
      if (!runtime) throw new Error("AgentAegis has not been initialized");
      ...
      const toolEvents = () => normalizeHermesTool(payload.tool_name,
        payload.args ?? {}, { home, skillDir: payload.skill_dir, workspaceDir: payload.workspace_dir });
      switch (method) {
        case "pre_llm_call": {
          const content = typeof payload.user_message === "string" ? payload.user_message : "";
          if (sessionKey) deniedSessions.delete(sessionKey);
        ...
        case "pre_tool_call":
          ...
          for (const event of toolEvents()) {
            ...
            if (result?.block) return { action: "block", message: result.blockReason };
          }
          return null;
        case "post_tool_call":
          for (const event of toolEvents()) {
        ...
        case "transform_tool_result": {
          ...
          const rewritten = (result?.message ?? message) as typeof message;
        ...
        case "transform_llm_output": {
          if (sessionKey && deniedSessions.has(sessionKey)) return deniedSessions.get(sessionKey);
