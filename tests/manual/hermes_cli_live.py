...
(home/'config.yaml').write_text(yaml.safe_dump({'model':{'provider':'anthropic','default':'claude-sonnet-5'},'terminal':{'backend':'local','cwd':str(work)},'plugins':{'enabled':['agent-aegis'],'entries':{'agent-aegis':{'settings':{'promptGuardEnabled':False,'dispatchGuardEnabled':False,'skillScanEnabled':False,'defaultBlockingMode':'enforce','protectedPaths':[str(canary)]}}}},'tools':{'tool_search':{'enabled':'off'}}}))
